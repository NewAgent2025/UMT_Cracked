...
namespace WiiMCRTool
{
    class Program
    {
        ...
        {
            Console.WriteLine("=== Wii MCR Extractor (AUTO+STRICT MODE | Wii U ZLIB) ===");
            ...
            else
            {
                ...
                // AUTO DETECT MODE
                // =========================
                string baseFolder = Path.Combine(Directory.GetCurrentDirectory(), "..", "UMT_Cracked_Convertion");
                ...
                var WiiFolders = Directory.GetDirectories(baseFolder)
                    .Where(d => Path.GetFileName(d).Contains("_Wii"))
                ...
                {
                    Console.WriteLine("❌ No _Wii folders found.");
                    return;
                }

                inputFolder = WiiFolders.First().FullName;
        ...
        static void ExtractMCR(string mcrPath, string outFolder)
        {
            ...
            using (FileStream fs = new FileStream(mcrPath, FileMode.Open, FileAccess.Read))
            {
                ...
                for (int i = 0; i < 1024; i++)
                {
                    ...
                    // ==============================
                    // DECOMPRESS (Wii U ZLIB)
                    // ==============================
                    byte[] data = DecompressWiiUChunk(raw);
        ...
        // =========================
        // WII U DECOMPRESS (ZLIB ONLY)
        // =========================
        static byte[] DecompressWiiUChunk(byte[] rawChunk)
        {
            try
            {
                using (MemoryStream ms = new MemoryStream(rawChunk))
                {
                    ...
                    if (compSize <= 0 || compSize > rawChunk.Length)
                        return new byte[0];

                    byte[] compressed = new byte[compSize];
                    ms.Read(compressed, 0, compSize);
        ...
        // ZLIB DECOMPRESS
        // =========================
        static byte[] ZlibDecompress(byte[] input)
        {
            uint destLen = (uint)(input.Length * 20);
            byte[] dest = new byte[destLen];

            int err = uncompress(dest, ref destLen, input, (uint)input.Length);
            if (err != 0)
                throw new Exception($"zlib error: {err}");

            byte[] result = new byte[destLen];
            Array.Copy(dest, result, destLen);
            return result;
        }
