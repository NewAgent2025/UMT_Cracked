...
namespace Full_UMT_Convertor.MCSBCode
{
	...
	public class AquaticConverter
	{
		// Token: 0x060004E1 RID: 1249 RVA: 0x00027D44 File Offset: 0x00025F44
		public MemoryStream ConvertToAquatic(TagNodeCompound inChunk)
		{
		...
		// Token: 0x060004E2 RID: 1250 RVA: 0x000280AC File Offset: 0x000262AC
		public TagNodeCompound ConvertFromAquatic(MemoryStream stream)
		{
			new NibbleSetters();
			AquaticParser aquaticParser = new AquaticParser();
			aquaticParser.ParseChunk(stream);
			AquaticChunkData aquaticChunk = aquaticParser.AquaticChunk;
			int x = aquaticChunk.X;
			int z = aquaticChunk.Z;
			byte[] blocks = aquaticChunk.Blocks;
			byte[] d = new byte[32768];
			byte[] d2 = new byte[32768];
			byte[] submerged = aquaticChunk.Submerged;
			TagNodeList value = aquaticChunk.NBTData.ContainsKey("Entities") ? (aquaticChunk.NBTData["Entities"].Copy() as TagNodeList) : new TagNodeList(TagType.TAG_COMPOUND);
			TagNodeList value2 = aquaticChunk.NBTData["TileEntities"].Copy() as TagNodeList;
			TagNodeList value3 = new TagNodeList(TagType.TAG_COMPOUND);
			TagNodeByteArray value4 = new TagNodeByteArray(aquaticChunk.HeightMap.ToArray<byte>());
			TagNodeByteArray value5 = new TagNodeByteArray(aquaticChunk.Biomes.ToArray<byte>());
			TagNodeCompound tagNodeCompound = new TagNodeCompound();
			tagNodeCompound.Add("Blocks", new TagNodeByteArray(blocks));
			...
			tagNodeCompound.Add("BlockLight", new TagNodeByteArray(d));
			tagNodeCompound.Add("SkyLight", new TagNodeByteArray(d2));
			...
			tagNodeCompound.Add("Submerged", new TagNodeByteArray(submerged));
			return new NbtTree
			{
				Root = 
				{
